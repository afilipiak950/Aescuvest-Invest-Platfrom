...
async function completeRemainingAISummaries() {
  const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
  
  // Process documents 797, 798, 799 (797 is currently processing, 798-799 are pending)
  const docsToProcess = [
    { docId: 797, jobId: 28, name: "Case Study - Personify Health (LG).pdf" },
    { docId: 798, jobId: 29, name: "Case Study - Primeyachts (LG).pdf" },
    { docId: 799, jobId: 30, name: "Intellywave_Unternehmensübersicht.pdf" }
  ];
  
  for (const { docId, jobId, name } of docsToProcess) {
    try {
      ...
      await db.update(backgroundJobs)
        .set({ 
          status: 'processing',
          progress: 75,
          currentStep: 'Generating comprehensive AI analysis...',
          startedAt: new Date()
        })
...
Focus on investment-relevant insights, financial implications, and strategic considerations. Be specific and actionable.`;
      ...
      await db.update(backgroundJobs)
        .set({
          status: 'completed',
          progress: 100,
          currentStep: 'AI analysis completed successfully',
          completedAt: new Date(),
          result: { success: true, aiSummary }
        })
        .where(eq(backgroundJobs.id, jobId));

      console.log(`✅ Completed comprehensive AI summary for document ${docId} (${name})`);
  ...
  console.log('All remaining AI summary processing completed');
}

completeRemainingAISummaries();