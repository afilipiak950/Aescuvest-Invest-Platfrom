...
export const systemKpis = pgTable("system_kpis", {
  id: serial("id").primaryKey(),
  ...
  aiAnalysesCompleted: integer("ai_analyses_completed").notNull().default(0),
  averageProcessingTime: numeric("average_processing_time", { precision: 10, scale: 2 }).default("0"),
  successRate: numeric("success_rate", { precision: 5, scale: 2 }).default("0"),
  monthlyRevenue: numeric("monthly_revenue", { precision: 15, scale: 2 }).default("0"),
  userActivity: integer("user_activity").notNull().default(0),
...
export const userActivity = pgTable("user_activity", {
  ...
  details: json("details").$type<{ [key: string]: any }>(),
...
export const aiActivity = pgTable("ai_activity", {
  ...
  activityType: text("activity_type").notNull(), // 'ocr', 'analysis', 'summary', 'evaluation'
  agentType: text("agent_type"), // 'clinical', 'legal', 'financial', etc.
  ...
  model: text("model"), // 'gpt-4o', 'mistral-7b', etc.
  prompt: text("prompt"),
...
export const reminders = pgTable("reminders", {
  ...
  type: text("type").notNull(), // 'deal_followup', 'document_review', 'system_maintenance', 'ai_analysis'
  priority: text("priority").notNull().default("medium"), // 'low', 'medium', 'high', 'urgent'
  status: text("status").notNull().default("pending"), // 'pending', 'completed', 'dismissed', 'snoozed'
...
export const performanceMetrics = pgTable("performance_metrics", {
  id: serial("id").primaryKey(),
  date: timestamp("date").defaultNow().notNull(),
  dealsPipeline: json("deals_pipeline").$type<{
  ...
  documentStats: json("document_stats").$type<{
  ...
  aiPerformance: json("ai_performance").$type<{
    totalQueries: number;
  ...
  userEngagement: json("user_engagement").$type<{
    activeUsers: number;
...
export const dashboardData = pgTable("dashboard_data", {
  id: serial("id").primaryKey(),
  dataType: text("data_type").notNull(), // 'kpi', 'chart', 'alert', 'notification'
  key: text("key").notNull(),
  value: json("value").$type<{ [key: string]: any }>(),
...
export const insertAiActivitySchema = createInsertSchema(aiActivity).omit({
  id: true,
  timestamp: true,
});
