...
async function getEmbeddingService() {
  const module = await import('./server/services/embeddingService.js');
  const EmbeddingService = module.EmbeddingService;
...
async function embedAllDocuments() {
  ...
  for (let i = 0; i < totalDocs; i += batchSize) {
    ...
    for (const doc of batch) {
      ...
      try {
        console.log(`[${progress}%] Embedding document ${doc.id}: ${doc.name.substring(0, 50)}...`);
...
embedAllDocuments()
  .then(() => {
    ...
    console.log('📝 Now running comprehensive tests...');
    process.exit(0);
  })
  .catch(error => {
