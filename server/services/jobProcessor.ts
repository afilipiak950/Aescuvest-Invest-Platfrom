...
class JobProcessor {
  ...
  /**
   * Detect if a file path is a cloud storage (GCS) path
   * IMPORTANT: Local files take priority - only treat as GCS if file doesn't exist locally
   */
  private isCloudStoragePath(filePath: string): boolean {
    // Explicit GCS protocol - always cloud
    if (filePath.startsWith('gs://')) return true;
    
    // If file exists locally, it's NOT a cloud path
    if (fs.existsSync(filePath)) return false;
    
    // Also check with process.cwd() prefix for relative paths
    const absolutePath = path.join(process.cwd(), filePath);
    if (fs.existsSync(absolutePath)) return false;
    
    // File doesn't exist locally - check for GCS patterns
    // These patterns indicate the path was generated by GCS upload flow
    if (filePath.startsWith('uploads/') && filePath.includes('deal-')) return true;
    if (filePath.startsWith('deals/')) return true;
    if (filePath.startsWith('chunked/')) return true;
  ...
  private async processDocumentOCR(job: BackgroundJob) {
    ...
    // Declare tempFilePath at top so finally block can always clean up
    ...
    try {
      ...
      // Resolve file path - try local first, then GCS
      let actualFilePath = filePath;
      if (filePath.startsWith('db://')) {
        // Database-stored file
        ...
        // Explicit GCS protocol - always download from cloud
        console.log(`📥 Explicit GCS path: ${filePath}, downloading...`);
        await this.updateJobProgress(job.id, 12, 'Downloading file from cloud storage...');
        ...
        tempFilePath = path.join(tempDir, `gcs_${Date.now()}_${fileName}`);
        await gcsService.downloadFile(filePath, tempFilePath);
        actualFilePath = tempFilePath;
        
        console.log(`✅ Downloaded GCS file to: ${tempFilePath}`);
        await this.updateJobProgress(job.id, 18, 'File downloaded from cloud storage, starting OCR...');
      } else {
        // Try to resolve locally first using validateAndResolvePath
        try {
          actualFilePath = await this.validateAndResolvePath(filePath);
          console.log(`✅ Resolved local file: ${actualFilePath}`);
        } catch (localError) {
          // Local resolution failed - try GCS as fallback
          console.log(`📥 Local file not found, trying GCS: ${filePath}`);
          await this.updateJobProgress(job.id, 12, 'Downloading file from cloud storage...');
          ...
          try {
            ...
            // Both local and GCS failed
            console.error(`❌ File not found locally or in GCS: ${filePath}`, gcsError);
            throw new Error(`File not found: ${filePath} (checked local storage and cloud storage)`);
          }
        }
      }
    ...
    const result = {
      ...
      // ALWAYS clean up temporary file, even on errors
