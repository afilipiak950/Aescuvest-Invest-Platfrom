...
export class ClaudeQuotaManager {
  ...
  async makeRequest<T>(
    ...
    const { fallbackContent, priority = 'medium', description = 'Claude request' } = options;
    ...
    for (let attempt = 1; attempt <= this.config.maxRetries; attempt++) {
      try {
        ...
        const isOverloadedError = this.isOverloadedError(error);
        ...
        if (attempt === this.config.maxRetries) {
          ...
          if (this.config.fallbackEnabled && fallbackContent) {
            ...
            throw new Error(`Claude request failed after ${this.config.maxRetries} attempts: ${error.message}`);
          }
        }

        // Calculate delay for next attempt
        const delay = this.calculateDelay(attempt, isQuotaError || isRateLimitError || isOverloadedError);
  ...
  private isRateLimitError(error: any): boolean {
    // Claude rate limit errors (different from quota)
    return error?.status === 429 && 
  ...
  private isOverloadedError(error: any): boolean {
    // Claude overloaded/capacity errors
  ...
  private handleQuotaExceeded(error: any): void {
    ...
    console.log(`🚫 Claude quota exceeded. Estimated reset: ${this.quotaStatus.estimatedResetTime?.toISOString()}`);
  }
  ...
  private handleOverloaded(error: any): void {
    console.log(`🔥 Claude overloaded, will retry with exponential backoff`);
  }
  ...
  reset(): void {
    this.quotaStatus = { isExceeded: false };
    this.failedRequestCount = 0;
    console.log(`🔄 Claude quota manager reset`);
  }
}

// Global quota manager instance
export const claudeQuotaManager = new ClaudeQuotaManager({
  maxRetries: 3,
  baseDelay: 5000,
  maxDelay: 300000,
  fallbackEnabled: true
