...
export class ComprehensiveFinancialAnalysisService {
  ...
  async startComprehensiveAnalysis(dealId: number, jobId?: string): Promise<void> {
    try {
      ...
      for (let i = 0; i < COMPREHENSIVE_FINANCIAL_QUESTIONS.length; i++) {
        ...
        const progress = Math.round((i / COMPREHENSIVE_FINANCIAL_QUESTIONS.length) * 100);
  ...
  private async getAssignedDocuments(dealId: number): Promise<any[]> {
    ...
    try {
      ...
      const financialKeywords = ['financial', 'revenue', 'profit', 'budget', 'forecast', 'cash', 'investment', 'valuation', 'ebitda', 'margin', 'p&l'];
      
      const prioritized = allDocs.map((doc: any) => ({
        doc,
        priority: financialKeywords.some(kw => 
      ...
      console.log(`📊 Using ${prioritized.length} prioritized financial documents for analysis`);
      return prioritized;
    } catch (error) {
...
CRITICAL: Extract ALL specific details from the AI SUMMARY CONTENT above (revenue, margins, burn rate, runway, valuation metrics). Respond in JSON:
{
  "answer": "Detailed extraction with specific financial data and metrics from the AI summaries",
  "confidence": 0-100,
...
"The financial analysis reveals the following:

• **2024 Revenue**: **$5.2M ARR** with **35% gross margin**

• **Burn Rate**: **$800K monthly** providing **18 months runway**

• **Funding History**: **$15M Series A** at **$50M pre-money valuation**"

WRONG (inline bullets - NEVER DO THIS):
"• Revenue: $5.2M • Margin: 35% • Burn: $800K"
  ...
  async getAllQuestionProgress(dealId: number): Promise<Record<string, number>> {
    ...
    const jobs = await db.query.backgroundJobs.findMany({
      where: and(
        ...
        eq(backgroundJobs.jobType, 'financial_question_rerun')
      )
    });
