...
export interface IStorage {
  ...
  // Microsoft email connection methods
  getMicrosoftEmailConnection(): Promise<any | undefined>;
  saveMicrosoftEmailConnection(connection: any): Promise<any>;
  clearMicrosoftEmailConnection(): Promise<any | undefined>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async deleteDocuments(fileIds: number[]): Promise<number> {
    const result = await db
      ...
      .where(
        fileIds.length === 1 
          ? eq(documents.id, fileIds[0])
          : inArray(documents.id, fileIds)
      );
  ...
  async getCompanyResearchByDealIdBackup(dealId: number): Promise<any | undefined> {
  ...
  async getDataRoomConnectionByDealId(dealId: number): Promise<any | undefined> {
    const [connection] = await db
      .select()
      .from(dataRoomConnections)
      .where(and(eq(dataRoomConnections.dealId, dealId), eq(dataRoomConnections.isActive, true)));
    return connection;
  }

  async createDataRoomConnection(connection: any): Promise<any> {
    const [newConnection] = await db
      .insert(dataRoomConnections)
      .values(connection)
      .returning();
  ...
  async updateDataRoomConnection(id: number, data: any): Promise<any | undefined> {
    const [connection] = await db
      .update(dataRoomConnections)
      .set({ ...data, updatedAt: new Date() })
      .where(eq(dataRoomConnections.id, id))
      .returning();
    return connection;
  }
  ...
  async disconnectDataRoom(dealId: number): Promise<any | undefined> {
    const [connection] = await db
      .update(dataRoomConnections)
      .set({ isActive: false, status: 'disconnected', updatedAt: new Date() })
      .where(eq(dataRoomConnections.dealId, dealId))
      .returning();
    return connection;
  }

  // Microsoft email connection methods for permanent database storage
  async getMicrosoftEmailConnection(): Promise<MicrosoftEmailConnection | undefined> {
    try {
      const [connection] = await db.select().from(microsoftEmailConnections).limit(1);
      return connection;
    } catch (error) {
      console.error('[Storage] Error getting Microsoft email connection:', error);
      return undefined;
    }
  }
  ...
  async saveMicrosoftEmailConnection(connection: InsertMicrosoftEmailConnection): Promise<MicrosoftEmailConnection> {
    try {
      ...
      console.log('[Storage] Microsoft email connection saved to database permanently');
      return savedConnection;
    } catch (error) {
      console.error('[Storage] Error saving Microsoft email connection:', error);
      throw error;
    }
  }

  async clearMicrosoftEmailConnection(): Promise<MicrosoftEmailConnection | undefined> {
    try {
      const [previous] = await db.select().from(microsoftEmailConnections).limit(1);
      await db.delete(microsoftEmailConnections);
      console.log('[Storage] Microsoft email connection cleared from database');
      return previous;
    } catch (error) {
      console.error('[Storage] Error clearing Microsoft email connection:', error);
      return undefined;
    }
  }
}
