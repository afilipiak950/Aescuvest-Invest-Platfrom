  ...
  async runComprehensiveAnalysis(dealId: number, storageService: any, jobId: string): Promise<any> {
    ...
    try {
      ...
      for (let i = 0; i < COMPREHENSIVE_CLINICAL_QUESTIONS.length; i++) {
        ...
        const progress = Math.round((i / COMPREHENSIVE_CLINICAL_QUESTIONS.length) * 100);
        await storageService.updateBackgroundJob(jobId, {
          progress,
...
CRITICAL: Extract ALL specific details from the AI SUMMARY CONTENT above (trial phases, patient numbers, efficacy metrics, safety data). Respond in JSON:
{
  ...
  "sources": ["doc1", "doc2"]
    ...
    const synthesisPrompt = `You are a senior clinical analyst. Synthesize these partial analyses into ONE comprehensive answer.
...
1. Extract ALL specific details (trial phases, patient numbers, efficacy/safety metrics) from all batches above
2. List ALL trials/studies with complete data
3. Provide exhaustive breakdown of clinical outcomes, regulatory status, and safety profile
4. Cite specific document sections and clinical data points
...
Respond in JSON:
{
  "answer": "START IMMEDIATELY WITH ANALYSIS - NO QUESTION PREFIX (Comprehensive synthesis with ALL specific clinical details formatted with markdown bullets and bold for key metrics)",
  "confidence": 0-100,
