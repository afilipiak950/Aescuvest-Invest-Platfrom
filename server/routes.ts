...
export async function registerAllRoutes(app: Express) {
  ...
  app.post('/api/deals/:dealId/upload/negotiate', async (req: Request, res: Response) => {
    try {
      ...
      if (fileSize > CLOUD_RUN_LIMIT) {
        ...
        // Small file -> Use direct upload (existing route)
        console.log(`📤 File <=30MB - using direct upload`);
        
        return res.json({
          success: true,
          uploadMethod: 'direct',
          uploadUrl: `/api/deals/${dealId}/data-room/upload-zip`,
          maxFileSize: CLOUD_RUN_LIMIT,
          instructions: {
            method: 'POST multipart/form-data',
            field: 'zipFile',
            additionalFields: ['folderName (optional)']
          }
        });
      }
    } catch (error) {
      console.error('❌ Upload negotiation failed:', error);
      return res.status(500).json({
        success: false,
  ...
  app.post('/api/deals/:dealId/upload/gcs-callback', async (req: Request, res: Response) => {
    try {
      ...
      wsManager.notifyProgress(dealId, {
        type: 'gcs_zip_extract',
        jobId,
        status: 'queued',
        progress: 0,
        message: `ZIP extraction queued for ${fileName}`,
        fileName
      });
