...
${document.ocrText ? document.ocrText.substring(0, 15000) : 'No OCR text available'}`
  ...
  // Market Position Search API endpoint
  app.post('/api/deals/:dealId/market-position-search', async (req: Request, res: Response) => {
    try {
      const { dealId } = req.params;
      const { companyName } = req.body;
      
      console.log(`📊 Starting market position search for ${companyName} (deal ${dealId})`);
      ...
      // Conduct market position analysis using OpenAI
      const response = await openai.chat.completions.create({
        model: "gpt-4o",
        messages: [
          {
            role: "system",
            content: "You are a market research analyst. Analyze the company's market position and provide detailed insights about their industry position, market size, and competitive advantages. Return results in valid JSON format."
          },
          {
            role: "user",
            content: `Analyze the market position of ${companyName}. Provide detailed analysis including:
...
Focus on publicly available information. If specific data is not available, indicate "Not publicly available" rather than guessing.`
      ...
      console.log(`✅ Market position search completed for ${companyName}`);
      ...
      console.error('Error conducting market position search:', error);
      res.status(500).json({ 
        success: false, 
        message: 'Failed to conduct market position search',
        error: error.message
      });
    }
  });

  // Competitive Landscape Search API endpoint
  app.post('/api/deals/:dealId/competitive-landscape-search', async (req: Request, res: Response) => {
    try {
      const { dealId } = req.params;
      const { companyName } = req.body;
      
      console.log(`🎯 Starting competitive landscape search for ${companyName} (deal ${dealId})`);
      ...
      // Conduct competitive landscape analysis using OpenAI
      const response = await openai.chat.completions.create({
        model: "gpt-4o",
        messages: [
          {
            role: "system",
            content: "You are a competitive intelligence analyst. Analyze the competitive landscape for the company and provide detailed insights about competitors, market dynamics, and competitive positioning. Return results in valid JSON format."
          },
          {
            role: "user",
            content: `Analyze the competitive landscape for ${companyName}. Provide detailed analysis including:
...
5. Key differentiators compared to competitors
...
Return the analysis in this JSON format:
{
  ...
  "marketShare": "market share context or 'Not publicly available'",
...
Focus on publicly available information. If specific data is not available, indicate "Not publicly available" rather than guessing.`
      ...
      const competitiveData = JSON.parse(response.choices[0].message.content || '{}');
      
      console.log(`✅ Competitive landscape search completed for ${companyName}`);
      ...
      console.error('Error conducting competitive landscape search:', error);
      res.status(500).json({ 
        success: false, 
        message: 'Failed to conduct competitive landscape search',
        error: error.message
      });
    }
  });
