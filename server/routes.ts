...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ========================================================================
  // 🚨 CRITICAL: AGENT RUN QUEUE ENDPOINTS - MUST BE REGISTERED EARLY
  // These need to be registered BEFORE Vite middleware to avoid HTML injection
  // ========================================================================
  console.log('🔄 Registering Agent Run Queue endpoints EARLY...');
  ...
  app.get('/api/deals/:dealId/agent-run-queue/status', async (req: Request, res: Response) => {
    console.log(`🎯 Agent run queue status hit: ${req.originalUrl}`);
    try {
  ...
  console.log('✅ Agent Run Queue endpoints registered EARLY');
  ...
  // OLD DUPLICATE ROUTE REMOVED - Use /api/deals/:dealId/generate-memo instead (line 6999)
...
${document.ocrText && typeof document.ocrText === 'string' ? document.ocrText.substring(0, 15000) : 'No OCR text available'}`
  ...
  app.post('/api/deals/:dealId/legal-analysis/comprehensive', async (req: Request, res: Response) => {
    try {
      ...
      // Import the CORRECT comprehensive legal analysis service
      const { comprehensiveLegalAnalysisService } = await import('./comprehensiveLegalAnalysisService');
      
      // Run comprehensive legal analysis in background with evidence extraction
      (async () => {
        try {
          console.log(`🔬 Starting comprehensive legal analysis background process for deal ${dealId}`);
          await comprehensiveLegalAnalysisService.runFullAnalysis(dealId);
      ...
      res.json({ 
        success: true, 
        message: 'Comprehensive legal analysis started - analyzing all 13 legal questions with evidence extraction from ALL assigned documents'
      });
    } catch (error) {
  ...
  app.post('/api/deals/:dealId/clinical-analysis/comprehensive', async (req: Request, res: Response) => {
    try {
      ...
      // Import the CORRECT comprehensive clinical analysis service
      const { comprehensiveClinicalAnalysisService } = await import('./comprehensiveClinicalAnalysisService');
      
      // Run comprehensive clinical analysis in background with evidence extraction
      (async () => {
        try {
          console.log(`🔬 Starting comprehensive clinical analysis background process for deal ${dealId}`);
          await comprehensiveClinicalAnalysisService.runFullAnalysis(dealId);
      ...
      res.json({ 
        success: true, 
        message: 'Comprehensive clinical analysis started - analyzing all clinical questions with evidence extraction from ALL assigned documents'
      });
    } catch (error) {
  ...
  app.post('/api/deals/:dealId/research-analysis/comprehensive', async (req: Request, res: Response) => {
    try {
      ...
      // Import the CORRECT comprehensive research analysis service
      const { comprehensiveResearchAnalysisService } = await import('./comprehensiveResearchAnalysisService');
      
      // Run comprehensive research analysis in background with evidence extraction
      (async () => {
        try {
          console.log(`🔬 Starting comprehensive research analysis background process for deal ${dealId}`);
          await comprehensiveResearchAnalysisService.runFullAnalysis(dealId);
      ...
      res.json({ 
        success: true, 
        message: 'Comprehensive research analysis started - analyzing all research questions with evidence extraction from ALL assigned documents'
      });
    } catch (error) {
  ...
  app.post('/api/deals/:dealId/financial-analysis/comprehensive', async (req: Request, res: Response) => {
    try {
      ...
      // Import the CORRECT comprehensive financial analysis service
      const { comprehensiveFinancialAnalysisService } = await import('./comprehensiveFinancialAnalysisService');
      
      // Run comprehensive financial analysis in background with evidence extraction
      (async () => {
        try {
          console.log(`💰 Starting comprehensive financial analysis background process for deal ${dealId}`);
          await comprehensiveFinancialAnalysisService.runFullAnalysis(dealId);
      ...
      res.json({ 
        success: true, 
        message: 'Comprehensive financial analysis started - analyzing all financial questions with evidence extraction from ALL assigned documents'
      });
    } catch (error) {
  ...
  app.post('/api/deals/:dealId/generate-memo', async (req: Request, res: Response) => {
    try {
      ...
      if (existingMemoJob) {
        // STALE JOB DETECTION: Check if job is stuck (no heartbeat for 20+ minutes)
        const jobAge = Date.now() - new Date(existingMemoJob.updatedAt).getTime();
        const STALE_THRESHOLD = 20 * 60 * 1000; // 20 minutes
        
        if (jobAge > STALE_THRESHOLD) {
          // Stale job detected - mark as failed and proceed with new generation
          console.log(`⚠️ Stale job detected (${Math.floor(jobAge / 60000)} minutes old) - auto-failing job ${existingMemoJob.jobId}`);
          await storage.updateBackgroundJob(existingMemoJob.jobId, {
            status: 'failed',
            currentStep: 'Job timed out - no heartbeat detected for 20+ minutes',
            completedAt: new Date()
          });
          console.log(`✅ Stale job marked as failed, proceeding with new generation`);
          ...
          // Active job running - return status
          console.log(`⚠️ Investment memo generation already running for deal ${dealId} (Job: ${existingMemoJob.jobId}, age: ${Math.floor(jobAge / 60000)} min)`);
          return res.json({ 
            success: true, 
...
export async function registerAllRoutes(app: Express) {
  ...
  // NOTE: Upload negotiation and GCS callback endpoints have been moved to server/index.ts
  // to ensure they execute before setupVite() and bypass Vite middleware interference.
  // See server/index.ts lines 1140-1278 for the active implementations.
  ...
  app.get('/api/deals/:dealId/ai-assistant/stats', async (req: Request, res: Response) => {
    try {
      ...
      console.log(`📊 Getting AI Assistant stats for deal ${dealId}`);
      ...
      res.status(500).json({ 
        error: 'Failed to get AI Assistant stats',
        message: error instanceof Error ? error.message : 'Unknown error'
      });
    }
  });
  ...
  // Note: Agent Run Queue endpoints are now registered EARLY in the file (around line 500)
  // to avoid Vite HTML injection issues
