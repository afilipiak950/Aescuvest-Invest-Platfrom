...
export function registerApiRoutes(app: Express) {
  ...
  app.post('/api/v1/deals/:dealId/memo/export-pdf', authenticateApiKey, async (req: Request, res: Response) => {
    try {
      const dealId = parseInt(req.params.dealId);
      
      if (isNaN(dealId)) {
        return res.status(400).json(apiResponse.error('Invalid deal ID provided'));
      }
      ...
      res.status(500).json(apiResponse.error('Failed to export investment memo as PDF'));
    }
  });

  // Export investment memo as Word document
  app.post('/api/v1/deals/:dealId/memo/export-docx', authenticateApiKey, async (req: Request, res: Response) => {
    try {
      const dealId = parseInt(req.params.dealId);
      ...
      res.status(500).json(apiResponse.error('Failed to export investment memo as Word document'));
    }
  });
