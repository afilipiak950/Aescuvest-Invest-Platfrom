...
async function seed() {
  console.log("🎯 Seeding unified agent questions...");
  ...
  try {
    ...
    if (existing.length > 0) {
      console.log("✅ Unified agent questions already exist");
      return;
    }
    ...
    for (const question of allQuestions) {
      await db.insert(unifiedAgentQuestions).values({
        ...
        category: "default",
        questionId: `${question.agentType}-q${question.priority}`,
        question: question.text,
        subQuestions: [],
        ragQueries: [question.text], // Use the question as the default RAG query
        priority: question.priority,
        isActive: true
      });
    }
    
    console.log(`✅ Seeded ${allQuestions.length} unified agent questions`);
  } catch (error) {
    console.error("❌ Error seeding unified agent questions:", error);
    throw error;
  }
}
...
export { seed as seedUnifiedQuestions };