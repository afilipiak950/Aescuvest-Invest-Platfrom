...
export class ComprehensiveCommercialAnalysisService {
  ...
  async getAssignedCommercialDocuments(dealId: number): Promise<any[]> {
    ...
    try {
      ...
      const commercialKeywords = ['commercial', 'market', 'sales', 'revenue', 'customer', 'pricing', 'competition', 'partnership', 'distribution', 'strategy'];
      
      const prioritized = allDocs.map((doc: any) => ({
        doc,
        priority: commercialKeywords.some(kw => 
      ...
      console.log(`📊 Using ${prioritized.length} prioritized commercial documents for analysis`);
      return prioritized;
    } catch (error) {
      console.error(`❌ Error loading commercial documents:`, error);
      return [];
    }
  }
    ...
    try {
      ...
      for (let i = 0; i < COMMERCIAL_QUESTIONS.length; i++) {
        ...
        try {
          ...
          const answer = await this.compileComprehensiveAnswer(question, documentEvidence, jobId, storageService, i, COMMERCIAL_QUESTIONS.length);
  ...
  private async extractEvidenceFromDocument(document: any, question: any): Promise<any> {
    ...
    // IMPORTANT: Process ALL documents - keywords are only for in-document relevance ranking
    // NOT for filtering which documents to analyze (matching Legal/Clinical behavior)
    console.log(`🔎 Extracting commercial evidence from: ${document.name}`);
...
CRITICAL: Extract ALL specific details from the AI SUMMARY CONTENT above (amounts, dates, metrics, percentages). Respond in JSON:
{
  "answer": "Detailed extraction with specific commercial data, metrics, and dates from the AI summaries",
  "confidence": 0-100,
...
Respond in JSON:
{
  ...
  private async storeComprehensiveResults(
    ...
    try {
      ...
      // Create the new comprehensive analysis - Use TypeScript property names (Drizzle auto-maps to DB columns)
      const analysisData = {
        dealId,
