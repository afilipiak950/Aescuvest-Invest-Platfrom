    ...
    <div className="min-h-screen bg-dark text-white">
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="w-full">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-3 pb-4">
          {stagesWithCounts.map((stage) => {
            const Icon = stage.icon;
            ...
            return (
              ...
              >
                {/* Stage Header */}
                <Card className="bg-dark-light border-dark-lighter mb-3">
                  <CardHeader className="p-3 pb-2">
                    <CardTitle className="flex items-center justify-between">
                      <div className="flex items-center gap-1">
                        <Icon className="h-4 w-4 text-gray-400" />
                        <span className="text-white text-sm font-semibold truncate">{stage.name}</span>
                      </div>
                      <Badge className={cn("border font-medium text-xs", stage.color)}>
                        {stage.count}
                      </Badge>
                    </CardTitle>
                ...
                <div className="space-y-3 min-h-[200px]">
                      ...
                      >
                        <CardContent className="p-2">
                          <div className="flex items-start gap-2 mb-2">
                            <div className={cn("flex-shrink-0 h-6 w-6 rounded flex items-center justify-center font-semibold text-white text-xs", bgColor)}>
                              {firstLetter}
                            </div>
                          ...
                          <div className="flex items-center justify-between mb-1 text-xs">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <span className={cn("text-xs", getAIScoreColor(deal.aiScore || 0))}>
                                {deal.aiScore || 0}
                              </span>
                            </div>
                          </div>
                          ...
                          <Link href={`/due-diligence?deal=${deal.id}`}>
                            <Button 
                              variant="ghost" 
                              size="sm" 
                              className="w-full text-primary hover:text-white hover:bg-primary/20 transition-all text-xs py-1 h-6"
                            >
                  ...
                  {/* Empty State */}
                  {stageDeals.length === 0 && (
                    <div className="border-2 border-dashed border-dark-lighter rounded-lg p-3 text-center">
                      <Icon className="h-6 w-6 text-gray-500 mx-auto mb-1" />
                      <p className="text-xs text-gray-500">No deals</p>
                      <p className="text-xs text-gray-600 mt-1">Drag here</p>
                    </div>
                  )}
                </div>
              </div>
