...
interface RunHRQueueButtonProps {
  dealId: number;
  onQueueStart?: () => void;
...
export function RunHRQueueButton({ 
  dealId, 
  onQueueStart, 
  onQueueComplete,
  onQuestionComplete,
    ...
    const checkQueueStatus = async () => {
      try {
        const [statusResponse, agentQueueResponse] = await Promise.all([
          apiRequest(`/api/deals/${dealId}/hr-analysis/queue-status`),
        ...
        if (agentQueueResponse.success && agentQueueResponse.queue) {
          const hrEntry = agentQueueResponse.queue.find(
            (entry: AgentRunQueueEntry) => entry.agentType === 'hr'
          );
          setAgentQueuePosition(hrEntry || null);
        } else {
          setAgentQueuePosition(null);
        }
      } catch (error) {
        console.error('Error checking HR queue status:', error);
      }
    };
  ...
  const handleForceRerunAll = async () => {
    setIsLoading(true);
    try {
      console.log(`🔥 FORCE RERUN: Starting ALL HR questions for deal ${dealId}`);
      
      const response = await apiRequest(`/api/deals/${dealId}/hr-analysis/force-rerun-all`, {
        method: 'POST',
      ...
      console.error('Error force rerunning HR questions:', error);
      
      toast({
        title: "Error",
        description: "Failed to force rerun all HR questions",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
  ...
  const handleCancelQueue = async () => {
    ...
    try {
      ...
      const response = await apiRequest(`/api/deals/${dealId}/hr-analysis/cancel-queue`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      });
      ...
      if (response.success) {
        toast({
          ...
          description: "HR analysis queue has been stopped",
        });
      } else {
        ...
        <CardContent className="pt-4">
          <div className="space-y-3">
            ...
            <p className="text-xs text-amber-200">
              HR analysis will start automatically when previous agents complete.
            </p>
          </div>
        </CardContent>
      </Card>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                variant="destructive"
                size="sm"
                data-testid="button-cancel-hr-queue"
              >
                {isLoading ? (
                  <>
  ...
  return (
    <Card className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 border-gray-700/30">
      <CardContent className="pt-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-white">HR Analysis</p>
            <p className="text-xs text-gray-400">Ready to analyze HR documents</p>
          </div>
          <Button
            onClick={handleForceRerunAll}
            disabled={isLoading}
            className={className}
            data-testid="button-force-rerun-hr"
          >
            {isLoading ? (
              <>
