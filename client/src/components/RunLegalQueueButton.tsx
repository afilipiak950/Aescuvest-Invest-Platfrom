    ...
    const checkQueueStatus = async () => {
      try {
        const [statusResponse, agentQueueResponse] = await Promise.all([
          apiRequest(`/api/deals/${dealId}/legal-analysis/queue-status`),
        ...
        if (agentQueueResponse.success && agentQueueResponse.queue) {
          const legalEntry = agentQueueResponse.queue.find(
            (entry: AgentRunQueueEntry) => entry.agentType === 'legal'
          );
          setAgentQueuePosition(legalEntry || null);
        } else {
        ...
        <CardContent className="pt-4">
          <div className="space-y-3">
            ...
            <p className="text-xs text-amber-200">
              Legal analysis will start automatically when previous agents complete.
            </p>
          </div>
        </CardContent>
      </Card>
  ...
  return (
    <Card className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 border-gray-700/30">
      <CardContent className="pt-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-white">Legal Analysis</p>
            <p className="text-xs text-gray-400">Ready to analyze legal documents</p>
          </div>
          <Button
