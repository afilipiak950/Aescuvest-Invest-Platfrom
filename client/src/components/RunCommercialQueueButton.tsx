...
export function RunCommercialQueueButton({ 
  ...
  // Poll queue status and agent run queue
  useEffect(() => {
    const checkQueueStatus = async () => {
      try {
        const [statusResponse, agentQueueResponse] = await Promise.all([
          apiRequest(`/api/deals/${dealId}/commercial-analysis/queue-status`),
        ...
        if (agentQueueResponse.success && agentQueueResponse.queue) {
          const commercialEntry = agentQueueResponse.queue.find(
            (entry: AgentRunQueueEntry) => entry.agentType === 'commercial'
          );
          setAgentQueuePosition(commercialEntry || null);
        } else {
  ...
  const handleForceRerunAll = async () => {
    setIsLoading(true);
    try {
      console.log(`🔥 FORCE RERUN: Starting ALL commercial questions for deal ${dealId}`);
      
      const response = await apiRequest(`/api/deals/${dealId}/commercial-analysis/force-rerun-all`, {
        method: 'POST',
  ...
  const handleCancelQueue = async () => {
    ...
    try {
      ...
      const response = await apiRequest(`/api/deals/${dealId}/commercial-analysis/cancel-queue`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      });
      ...
      if (response.success) {
        toast({
          ...
          description: "Commercial analysis queue has been stopped",
        });
      } else {
        ...
        <CardContent className="pt-4">
          <div className="space-y-3">
            ...
            <p className="text-xs text-amber-200">
              Commercial analysis will start automatically when previous agents complete.
            </p>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="pt-4">
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-white">
                  Processing Commercial Questions
                </p>
                <p className="text-xs text-gray-400">
  ...
  return (
    <Card className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 border-gray-700/30">
      <CardContent className="pt-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-white">Commercial Analysis</p>
            <p className="text-xs text-gray-400">Ready to analyze commercial documents</p>
          </div>
          <Button
            onClick={handleForceRerunAll}
            disabled={isLoading}
            className={className}
            data-testid="button-force-rerun-commercial"
          >
            {isLoading ? (
              <>
