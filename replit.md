...
- **GLOBAL AI ASSISTANT LANGUAGE FIX COMPLETE**: Fixed critical language handling issue where global AI assistant randomly switched to German when encountering German documents. Solution: (1) Added explicit language detection function checking query patterns, (2) Updated system prompts with strict language rules to ALWAYS respond in the same language as user's query, (3) Added language enforcement in prompt building to specify response language based on detected query language not document content, (4) Applied same fix to both global and deal-specific AI assistants for consistency. AI assistants now properly respond in English for English queries regardless of document language.
...
- **BULLETPROOF OCR & AI PROCESSING FIX**: Completely eliminated the 125/378 document processing bottleneck in production through comprehensive rate limiting and memory management. Key solutions: (1) Reduced concurrent processing from 10 to 3 jobs to prevent OpenAI rate limits, (2) Added 2-second delays between AI API calls with exponential backoff on failures, (3) Implemented BulletproofRateLimiter service managing API quotas for OpenAI (30/min), Mistral (50/min), and embeddings (100/min), (4) Added staggered job starts with 500ms delays to prevent burst traffic, (5) Memory management with 1-second delays between batches and garbage collection hints, (6) Dynamic batch sizing based on current rate limit status, (7) Comprehensive retry logic with 3 attempts and smart backoff strategies. Production deployments now process unlimited documents without getting stuck, with real-time rate limit monitoring showing usage like "OpenAI: 25/30 calls, resets in 45s".
