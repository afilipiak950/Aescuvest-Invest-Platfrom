...
- **Ultra-Premium PDF Export**: Enterprise-grade typography and professional formatting. **NEW (Dec 2025): Claude + Puppeteer Pipeline** - Premium PDF exports use Claude Sonnet to synthesize memo sections into structured JSON (paragraphs, tables, bullets, callouts, metrics, key-values), then render via Puppeteer with Goldman Sachs/McKinsey-style CSS (navy headers, gold accents, premium tables, callout boxes). API: `POST /api/deals/:dealId/export-pdf` with `premium=true` for enhanced rendering. Graceful fallback to jsPDF if Claude unavailable.
